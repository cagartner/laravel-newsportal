...
@if ($permissions->get()->count())
  ...
  <thead>
    <tr>
      <th colspan="12">تقرير اداء الاعضاء خاص بالاخبار بتاريخ {{$today}}</th>
    </tr>
    <tr>
  			<th colspan="">#</th>
    		<th colspan="2">اسم العضو</th>
			<th colspan="2">عنوان الخبر</th>
			<th colspan="2">عدد المشاهدات</th>
    	...
    	@if ($news->count())
			@foreach($news as $view)
				<tr>
					...
					<td colspan="2">{{$view->views}}</td>
					<td colspan="2">{{$view->created_at}}</td>
					<td colspan="2">{{$view->updated_at}}</td>
					<td colspan="2">
						@foreach ($creates_arr as $arr)
							<a href="/admin/report/news?title=&status=&created_at=&user_id={{$view->user_id}}">{{$arr}}</a>
							<?php break; ?>
						@endforeach
					</td>
				</tr>
